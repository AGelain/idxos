            ...
            <form action="<?php echo current_url(); ?>" id="formCliente" method="post" class="form-horizontal">
                <div class="widget-content nopadding tab-content">
                    <ul class="nav nav-tabs">
                        <li class="active" id="tabDetalhes"><a href="#tab1" data-toggle="tab">Detalhes do Cliente</a></li>
                        <li id="tabVeiculos"><a href="#tab2" data-toggle="tab">Veículos</a></li>
                    </ul>
                    <div class="tab-content">
                        ...
                        <div class="tab-pane" id="tab2">
                            <div class="span12 well" style="padding: 1%; margin-left: 0">
                                <form id="formVeiculos" action="<?php echo base_url() ?>index.php/clientes/adicionarVeiculo" method="post">
                                    <div class="span10">
                                        <input type="hidden" name="idClientes" id="idClientes" value="<?=$result->idClientes?>" />
                                        <input type="hidden" name="idVeiculos" id="idVeiculos" />
                                        <label for="">Veiculo</label>
                                        <input type="text" class="span12" name="veiculo" id="veiculo" placeholder="Digite a placa do veiculo" />
                                    </div>
                                    <div class="span2">
                                        <label for="">&nbsp;</label>
                                        <!-- <button type="button" id="send-veiculo" class="btn btn-success span12"><i class="fas fa-plus"></i> Adicionar</button> -->
                                        <button type="button" id="send-veiculo" class="button btn btn-mini btn-success">
                            ...
                            <div id="listaVeiculos">
                                <div class="widget-box" id="divVeiculos">
                                    <div class="widget_content nopadding">
                                        <table width="100%" class="table table-bordered" id="tblVeiculos">
                                            <thead>
                                                <tr>
                                                    <th>Cód.</th>
                                                    <th>Placa</th>
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($veiculos as $r) {
                                                    ...
                                                    echo '<td><span idAcao="' . $r->idVeiculos . '" title="Desvincular Veículo" class="btn-nwe4 veiculo"><i class="bx bx-trash-alt bx-xs"></i></span></td>';
                                                    echo '</tr>';
                                                } ?>
                                            </tbody>
                                        </table>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#send-veiculo").click(function() {
            var idVeiculos = $("#idVeiculos").val();
            var idClientes = "<?=$result->idClientes?>"

            if (!(idClientes > 0)) {
                Swal.fire({
                    type: "error",
                    title: "Atenção",
                    text: "Insira um veiculo"
                });
            }
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>index.php/clientes/vincularVeiculo",
                data: { idClientes, idVeiculos },
                dataType: 'json',
                success: function(data) {
                    console.log(data);
                    if (data.result == true) {
                        $("#listaVeiculos").load("<?php echo current_url(); ?> #divVeiculos");
                        $("#veiculo").val('').focus();
                    } else {
                        $("#listaVeiculos").load("<?php echo current_url(); ?> #divVeiculos");
                        $("#veiculo").val('').focus();
        ...
        $("#veiculo").autocomplete({
            source: "<?php echo base_url(); ?>index.php/veiculos/autoCompleteVeiculo",
            minLength: 2,
        ...
        $(document).on('click', '.veiculo', function(event) {
            var idClientes = <?php echo $result->idClientes ?>;
            var idVeiculos = $(this).attr('idAcao');
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>index.php/clientes/desvincularVeiculo",
                data: { idClientes, idVeiculos },
                dataType: 'json',
                success: function(data) {
                    if (data.result == true) {
                        ...
                        Swal.fire({
                            type: "error",
                            title: "Atenção",
                            text: "Ocorreu um erro ao tentar desvincular veiculo."
                        });
                    }
                }
            });
